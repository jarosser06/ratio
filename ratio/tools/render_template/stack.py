...
class RatioRenderTemplateTool(Stack):
    def __init__(self, app_name: str, app_base_image: str, architecture: str, deployment_id: str,
        ...
        self.tool_execute = EventBusSubscriptionFunction(
            ...
            construct_id="ratio-tool-render-template-execution",
            event_type="ratio::tool::render_template::execution",
            description="Template Renderer Tool",
            entry=self.runtime_path,
            index="run.py",
            handler="handler",
            function_name=resource_namer("tool-render-template", scope=self),
            memory_size=256,
