...
class RatioMathTool(Stack):
    def __init__(self, app_name: str, app_base_image: str, architecture: str, deployment_id: str,
        ...
        self.tool_execute = EventBusSubscriptionFunction(
            ...
            construct_id="ratio-tool-math-execution",
            event_type="ratio::tool::math::execution",
            description="Tool execution for Math tool",
            entry=self.runtime_path,
            index="run.py",
            handler="handler",
            function_name=resource_namer("tool-math", scope=self),
            memory_size=256,
