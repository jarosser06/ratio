"""
Reference validation module for agent instructions.
"""
...
from ratio.core.services.agent_manager.runtime.agent import AgentInstruction
...
class RefValidator:
    """
    Validates REF references in agent instructions to ensure type consistency.
    """
    ...
    def __init__(self):
        ...
        # Execution context mapping execution_ids to AgentInstruction objects
    ...
    def register_agent_instruction(self, execution_id: str, agent_instruction: AgentInstruction):
        """
        Register an agent instruction in the execution context.
        
        Keyword arguments:
        execution_id -- The execution ID of the instruction
        agent_instruction -- The instruction to register
        """
        self.execution_context[execution_id] = agent_instruction
    ...
    def get_ref_type(self, ref_string: str) -> str:
        ...
        if base_context == "arguments":
            ...
            current_agent = self.execution_context[current_execution_id]

            # Get argument type from schema
            arg_def = next((arg for arg in current_agent.definition.attribute_definitions 
            ...
            agent = self.execution_context[base_context]

            # Get response type from schema
            resp_def = next((resp for resp in agent.definition.response_definitions 
    ...
    def validate_instruction_argument(self, execution_id: str, arg_name: str, arg_value: Any) -> List[RefTypeError]:
        """
        Validate a single argument in an agent instruction for REF type consistency.
        ...
        agent = self.execution_context.get(execution_id)
        ...
        arg_def = next((arg for arg in agent.definition.attribute_definitions 
                    if arg.get("name") == arg_name), None)
    ...
    def validate_instruction(self, agent_instruction: AgentInstruction) -> List[RefTypeError]:
        """
        Validate all REF references in an agent instruction.

        Keyword arguments:
        agent_instruction -- The agent instruction to validate
        ...
        self.register_agent_instruction(execution_id, agent_instruction)

        # Validate each argument
        for arg_name, arg_value in agent_instruction.provided_arguments.items():
    ...
    def validate_instructions(self, instructions: List) -> Dict[str, List[RefTypeError]]:
        """
        Validate a list of agent instructions for REF type consistency.

        Keyword arguments:
        instructions -- List of agent instructions to validate
        ...
        for instruction in instructions:
            self.register_agent_instruction(instruction.execution_id, instruction)
