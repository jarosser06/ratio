...
Provides functionality for managing composite tools and determining the execution order of tools.
"""
import json
import logging
import os
...
class ExecutionEngine:
    def __init__(self, arguments: Dict[str, Any], process_id: str, token: str, working_directory: str,
        ...
        if not system_event_endpoint:
            ...
            if response_definition and not response_reference_map:
                raise ValueError("Response reference map must be provided if response definition is set for composite tools")
    ...
    def load_from_fs(cls, process_id: str, token: str, working_directory: str) -> "ExecutionEngine":
        ...
        proc_dir = f"tool_exec-{process_id}"
    ...
    def _expand_parallel_instruction(self, execution_id: str) -> List[str]:
        ...
        for i, item in enumerate(resolved_value):
            ...
            child_instruction = ToolInstruction(
    ...
    def _load_instructions(self, instructions: List[Dict[str, Any]], token: str) -> Dict[str, ToolInstruction]:
        """
        ...
        for instruction in instructions:
            ...
            if instruction.get("tool_definition_path"):
                tool_definition = ToolDefinition.load_from_fs(
                    tool_file_location=instruction["tool_definition_path"],
                    token=token,
                )
            else:
                tool_definition = ToolDefinition(**instruction["tool_definition"])

            # Just load the instruction as-is, don't expand parallel yet
            loaded_instructions[execution_id] = ToolInstruction(
    ...
    def close(self) -> Union[str, None]:
        """
        Write the final response to the file system for composite tools.

        Assumes the tool wrote it's own response to the file system if it was a Lambda based tool.
        """
        # If not a composite execution, the tool is responsible for writing the response
        if not self.is_composite:
            logging.info("Direct tool execution, tool responsible for writing response .. skipping")
    ...
    def mark_completed(self, execution_id: str, response_path: Optional[str] = None):
        ...
        # Check if the tool provided a response
        instruction = self.instructions[execution_id]
    ...
    def prepare_for_execution(self, tool_instruction: ToolInstruction, process_id: Optional[str] = None,
                              working_directory: Optional[str] = None) -> Union[str, None]:
        """
        Prepares the system to execute an tool. This creates the appropriate sub-directory in the
        execution directory and saves the tool definition to the file system.

        Keyword arguments:
        tool_instruction -- The instruction to execute
        ...
        tool_definition = tool_instruction.definition

        if not tool_definition.arguments:
            logging.debug("Tool takes no arguments .. skipping")
        ...
        provided_arguments = tool_instruction.provided_arguments
        ...
        if tool_instruction.transform_arguments:
            try:
                rendered_arguments = self._apply_transforms(
                    response=rendered_arguments, 
                    transform_config=tool_instruction.transform_arguments
                )
