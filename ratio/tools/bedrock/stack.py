...
class RatioBedrockTools(Stack):
    def __init__(self, app_name: str, app_base_image: str, architecture: str, deployment_id: str,
        ...
        self.text_execute = EventBusSubscriptionFunction(
            ...
            construct_id="ratio-tool-bedrock-text-execution",
            event_type="ratio::tool::bedrock::text::execution",
            description="Tool execution for Bedrock Text tool",
            entry=self.runtime_path,
            index="text.py",
            handler="handler",
            function_name=resource_namer("tool-bedrock-text", scope=self),
            memory_size=512,
            managed_policies=[
        ...
        self.cohere_embeddings_execute = EventBusSubscriptionFunction(
            base_image=self.app_base_image,
            construct_id="ratio-tool-bedrock-cohere-embedding-execution",
            event_type="ratio::tool::bedrock::cohere_embedding::execution",
            description="Tool execution for Bedrock Cohere Embeddings tool",
            entry=self.runtime_path,
            index="cohere.py",
            handler="handler",
            function_name=resource_namer("tool-bedrock-cohere-embedding", scope=self),
            memory_size=512,
            managed_policies=[
                ManagedPolicy.from_managed_policy_arn(
                    scope=self,
        ...
        self.titan_embeddings_execute = EventBusSubscriptionFunction(
            base_image=self.app_base_image,
            construct_id="ratio-tool-bedrock-titan-embeddings-execution",
            event_type="ratio::tool::bedrock::titan_embedding::execution",
            description="Tool execution for Bedrock Titan Embedding tool",
            entry=self.runtime_path,
            index="titan.py",
            handler="handler",
            function_name=resource_namer("tool-bedrock-titan-embedding", scope=self),
            memory_size=512,
            managed_policies=[
                ManagedPolicy.from_managed_policy_arn(
                    scope=self,
