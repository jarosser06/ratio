...
class RatioAgentInternalAPI(Stack):
    def __init__(self, app_name: str, app_base_image: str, architecture: str, deployment_id: str,
        ...
        self.agent_execute = EventBusSubscriptionFunction(
            ...
            construct_id="ratio-agent-internal-api-execution",
            event_type="ratio::agent::internal_api::execution",
            description="Agent that submits internal API requests to the Ratio API.",
            entry=self.runtime_path,
            index="run.py",
            handler="handler",
            function_name=resource_namer("agent-internal-api-execution", scope=self),
            memory_size=256,
