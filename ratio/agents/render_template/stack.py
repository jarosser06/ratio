...
class RatioRenderTemplateAgent(Stack):
    def __init__(self, app_name: str, app_base_image: str, architecture: str, deployment_id: str,
        ...
        self.agent_execute = EventBusSubscriptionFunction(
            ...
            construct_id="ratio-agent-render-template-execution",
            event_type="ratio::agent::render_template::execution",
            description="Template Renderer Agent",
            entry=self.runtime_path,
            index="run.py",
            handler="handler",
            function_name=resource_namer("agent-render-template", scope=self),
            memory_size=256,
