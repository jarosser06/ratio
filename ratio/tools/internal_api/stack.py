...
class RatioInternalAPITool(Stack):
    def __init__(self, app_name: str, app_base_image: str, architecture: str, deployment_id: str,
        ...
        self.tool_execute = EventBusSubscriptionFunction(
            ...
            construct_id="ratio-tool-internal-api-execution",
            event_type="ratio::tool::internal_api::execution",
            description="Tool that submits internal API requests to the Ratio API.",
            entry=self.runtime_path,
            index="run.py",
            handler="handler",
            function_name=resource_namer("tool-internal-api-execution", scope=self),
            memory_size=256,
