...
def general_event_handler(event, context):
    ...
    for subscription in subscriptions:
        ...
        # Validate agent definition access
        ...
        if validation_response.status_code == 404:
            logging.debug(f"Agent definition {subscription.agent_definition} not found")

            err_msg = f"agent definition path {subscription.agent_definition} for subscription {subscription.event_type} - {subscription.subscription_id} not found"
        ...
        if not entity_has_access:
            ...
            err_msg = f"entity {subscription.process_owner} does not have access to agent definition path {subscription.agent_definition} for subscription {subscription.event_type} - {subscription.subscription_id}"
        ...
        # Execute the agent
        agent_mgr = RatioInternalClient(service_name="agent_manager", token=token)
        ...
        try:
            ...
            agent_response = agent_mgr.request(path="/execute", request=agent_exec_req)
            ...
            err_msg = f"Agent execution failed for subscription {subscription.event_type} - {subscription.subscription_id} with error {agent_exec_error}"
