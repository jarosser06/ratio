"""
Handlers for the tool manager events.
"""
import json
import logging
import os
...
def _execute_children(claims: JWTClaims, execution_engine: ExecutionEngine, execution_ids: list,
    ...
    execution_engine -- The execution engine for the composite tool.
    ...
    for execution_id in execution_ids:
        ...
        if execution_engine.instructions[execution_id].definition.is_composite:
            ...
            if not definition_og_file_path:
                ...
                except ToolExportError as export_err:
                    # Handle export error
                    logging.debug(f"Error exporting tool definition to file {temp_definition_path}: {export_err}")

                    _close_out_process(
                        process=child_proc,
                        failure_reason=f"error exporting tool definition to file {temp_definition_path}: {export_err}",
                        skip_failure_notification=True,  # Skiping since the parent is handled right after
                        token=token,
                    )
...
def process_complete_handler(event: Dict, context: Dict):
    ...
    if not execution_ids and len(execution_engine.in_progress) == 0:
        logging.debug(f"No more executions for composite tool process: {parent_proc.process_id}")
...
_COMPOSITE_FN_NAME = "ratio.services.process.composite_tool_handler"


@fn_event_response(exception_reporter=ExceptionReporter(), function_name=_COMPOSITE_FN_NAME, logger=Logger(_COMPOSITE_FN_NAME))
def execute_composite_tool_handler(event: Dict, context: Dict):
    """
    Execute the tool
    """
    logging.debug(f"Received request: {event}")
    ...
    logging.debug(f"Executing tool with request body: {event_body.to_dict()}")
    ...
    if validate_file_access_response.status_code == 404:
        logging.debug(f"Tool definition path does not exist: {event_body["tool_definition_path"]}")

        _close_out_process(
            process=proc,
            failure_reason="tool definition path does not exist",
            token=token,
        )
    ...
    if not entity_has_access:
        logging.debug(f"Requestor does not have access to tool definition path: {event_body["tool_definition_path"]}")

        _close_out_process(
            process=proc,
            failure_reason="requestor does not have access to tool definition path",
            token=token,
        )
    ...
    try:
        tool_definition = ToolDefinition.load_from_fs(
            tool_file_location=event_body["tool_definition_path"],
            token=token,
        )
        ...
        _close_out_process(
            process=proc,
            failure_reason=f"error loading tool definition from file {e}",
            token=token,
        )
    ...
    if proc.parent_process_id != "SYSTEM":
        ...
        if parent_proc.execution_status in [ProcessStatus.COMPLETED, ProcessStatus.FAILED]:
            _close_out_process(
                process=proc,
                failure_reason="parent process is already closed, cannot execute tool",
                token=token,
            )
    ...
    if execution_engine.is_composite:
        ...
        if not execution_ids:
            ...
            _close_out_process(
                process=proc,
                failure_reason="no available executions for tool, likely due to invalid tool definition",
                token=token,
            )
