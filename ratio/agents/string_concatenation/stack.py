...
class RatioStringConcatenationAgent(Stack):
    def __init__(self, app_name: str, app_base_image: str, architecture: str, deployment_id: str,
        ...
        self.agent_execute = EventBusSubscriptionFunction(
            ...
            construct_id="ratio-agent-string-concatenation-execution",
            event_type="ratio::agent::string_concatenation::execution",
            description="Agent execution for String Concatenation agent",
            entry=self.runtime_path,
            index="run.py",
            handler="handler",
            function_name=resource_namer("agent-string-concatenation", scope=self),
            memory_size=256,
